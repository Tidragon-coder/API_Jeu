...
export default function GameListPage() {
  ...
  const handleAdminGameList = async (id: string, action: "delete" | "update", name?: string) => {
        try {
          const token = localStorage.getItem("token");
          if (!token) return setError({ code: 401, message: "Token manquant." });
    
          if (action === "delete"){
            if (!window.confirm(`Etes-vous sur de vouloir supprimer ${name} de votre GameList ?`)) return;
          }
          ...
          if (action === "delete"){
            console.log(id);
            await callApi(`/gamelist/${id}`, token, "DELETE")
          ...
          setGameList((prev) => prev.filter((genre) => genre._id !== id));
        } catch (err: unknown) {
          if (axios.isAxiosError(err)) {
  ...
  if (error.code)
    ...
    if (games.length === 0)
      return (
        <div className="flex justify-center mt-8">
          <p className="text-center mt-8 text-gray-600">Aucun jeu disponible.</p>
        </div>
      );
            ...
            {gameList.map((g) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-sm text-gray-500 flex space-x-4">
                  ...
                  <a href="#" onClick={() => handleAdminGameList(g._id, "delete", "ce jeu")}>
